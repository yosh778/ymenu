...
void GameApp::Create()
{
	...
	renderer->SetSwizzle(0);
	// Load background texture (texture format can be PNG, JPG or GIF)
	mBgTex = renderer->LoadTexture("bg.png", 0, 0, NULL, 0, false);
	...
	mSpriteTex = renderer->LoadTexture("planet.png");
	
	if (mSpriteTex == NULL)	sceKernelExitGame();
	...
	//if (mGirl == NULL || mBoy == NULL)	sceKernelExitGame();
