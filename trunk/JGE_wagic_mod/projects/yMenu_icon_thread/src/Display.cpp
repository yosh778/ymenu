...
void Display::FindBgs()
{
	...
	srand(time(NULL));
	#ifdef PSP
	vector<string>* dir = YDir::getDirNames("./");
...
{
	while ( YDir::getLITid_done() != 1 )	sceKernelDelayThread(1000);
...
void Display::updateNotify()
{
	...
	switch (mNotify)
	{
		case NOTIFICATION_ACTIVE:
			...
			else if (engine->GetButtonClick(PSP_CTRL_CROSS))	mChoice = true, mNotify = NOTIFICATION_WORKING, mNotifyMess = "Installing ...";
			break;
