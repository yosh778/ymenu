...
void YDir::Create()
{
	...
	if ( mLITid_done == 1 )
	{
		// Launch icon_load thread
		this->LoadIconThreadStart();
		mCurLIT_done = true;
	}
...
void YDir::Destroy()
{
	if ( !mCurLIT_done )
	{
		YLOG("Wait until end of loading\n");
		// Wait until end of loading
		if ( this->getLITid_done() != 1 )	this->setLITid_done(2);
		int waitTime = 0;
		while ( this->getLITid_done() != 1 && waitTime < 1000 )	waitTime++, sceKernelDelayThread(1000);
		if (mLITid_done != 1)
		{
			mLITid_done = 1;
			sceKernelTerminateThread(mLITid);
		}
		YLOG("Wait until end of loading_done\n");
		mCurLIT_done = true;
	}
...
vector<string>* YDir::getDirNames( string dirpath )
{
...
struct LoadIconThread_ {
	vector<YEntry>* folders;
	int curFolder;
	int* done;
};
...
int YDir_LoadIconThread(SceSize args, void *argp)
{
	YLOG("YDir_LoadIconThread in\n");
	LoadIconThread_* lIT = (LoadIconThread_*) argp;
	vector<YEntry>* folders = lIT->folders;
	int i;
	
	JRenderer* renderer = JRenderer::GetInstance();
	for ( i=0; i< (*folders).size() && (*lIT->done) != 2; ++i )
	{
		if ( (*folders)[i].inHbPath() )
		{
			(*folders)[i].findEboot();
			if ( (*folders)[i].containsEboot() )
			{
				(*folders)[i].setEboot ( new Eboot ( (*folders)[i].getEbootPath() ) );
				
				if ( ((*folders)[i].getEboot() != NULL) && ((*folders)[i].getEboot()->getPngData() != NULL) )
				{
					JTexture* tex = renderer->LoadTexture((*folders)[i].getEbootPath().c_str(), TEX_TYPE_USE_VRAM
																											#ifdef wagic
																											, TEXTURE_FORMAT,
																											(*folders)[i].getEboot()->getPngData()
																											#endif
																											);
					(*folders)[i].getEboot()->freePngData();
					
					if ( tex != NULL )
					{
						(*folders)[i].setIcon( new JQuad(tex, 0, 0, tex->mWidth, tex->mHeight) );
						(*folders)[i].setIconTex ( tex );
					}
					else
					{
						(*folders)[i].setIcon( DSystm::GetInstance()->getCorruptIcon() );
					}
				}
				else	(*folders)[i].setIcon ( DSystm::GetInstance()->getDefIcon() );
			}
			
			// If loading icon, change into folder icon
			if ((*folders)[i].getIcon() == DSystm::GetInstance()->getFolderIcon())	(*folders)[i].setIcon ( DSystm::GetInstance()->getFolderIcon() );
		}
		//else if (this->inSavePath())	this->findArchive();
		
		
		(*folders)[i].initDispName();
	}
	YLOG("i: %i\n(*lIT->done): %i\n", i, (*lIT->done));
...
void YDir::LoadIconThreadStart()
{
	
	if (mLITid < 0)
	{
		YLOG("LoadIconThreadStart create\n");
		mLITid = sceKernelCreateThread( "LoadIconThread", &YDir_LoadIconThread, 50, 0x1000, 0, NULL );
	}
	
	if (mLITid >= 0)
	{
		
		while ( Display::getSwapBgState() != -1 )	sceKernelDelayThread(1000);
		YLOG("LoadIconThreadStart start\n");
		mLITid_done = 0;
		LoadIconThread_ lIT = { &mFolders, mCurFolder, &mLITid_done };
		sceKernelStartThread(mLITid, sizeof(lIT), &lIT);
		YLOG("LoadIconThreadStart done\n", i, (*lIT->done));
	}
...
int YDir::getLITid_done()
{
	return mLITid_done;
}

void YDir::setLITid_done( int lITid_done )
{
	if ( lITid_done == 2 && mLITid_done == 0 )
	{
		mLITid_done = lITid_done;
		YLOG("setLITid_done success\n");
	}
	else	YLOG("setLITid_done error: mLITid_done == %i", mLITid_done);
}
