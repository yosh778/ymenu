...
void CmdExtract::DoExtract(CommandData *Cmd)
{
  ...
  if (TotalFileCount==0 && *Cmd->Command!='I')
  {
    ...
    if (!Cmd->DisableDone)
      if (*Cmd->Command=='I')
        dummy_printf(St(MDone));
      else
        if (ErrHandler.GetErrorCount()==0)
          dummy_printf(St(MExtrAllOk));
        else
          dummy_printf(St(MExtrTotalErr),ErrHandler.GetErrorCount());
#endif
}
...
EXTRACT_ARC_CODE CmdExtract::ExtractArchive(CommandData *Cmd)
{
  ...
  {
#ifndef GUI
    dummy_printf(St(MNotRAR),ArcName);
#endif
  ...
  if (*Cmd->Command=='I')
    ...
    if (Cmd->Test)
      dummy_printf(St(MExtrTest),ArcName);
    else
      dummy_printf(St(MExtracting),ArcName);
#endif
  ...
  if (ExactMatch || (SkipSolid=Arc.Solid)!=0)
  {
    ...
    if (!IsLink(Arc.NewLhd.FileAttr))
      if (Arc.IsArcDir())
      {
        ...
        {
#ifndef GUI
          dummy_printf(St(MExtrSkipFile),ArcFileName);
#endif
          return(true);
        }
        TotalFileCount++;
        ...
        {
#ifndef GUI
          dummy_printf(St(MExtrTestFile),ArcFileName);
          dummy_printf(" %s",St(MOk));
#endif
          return(true);
        }
        ...
        {
#ifndef GUI
          dummy_printf(St(MCreatDir),DestFileName);
          dummy_printf(" %s",St(MOk));
#endif
          PrevExtracted=true;
        }
...
#ifndef SFX_MODULE
            case 'P':
              dummy_printf(St(MExtrPrinting),ArcFileName);
              break;
#endif
            case 'X':
            case 'E':
              dummy_printf(St(MExtrFile),DestFileName);
              break;
          }
      ...
      if (!SkipSolid)
      {
            ...
            !Arc.OldFormat && UINT32(DataIO.UnpFileCRC)==UINT32(Arc.NewLhd.FileCRC^0xffffffff))
        {
          ...
          if (Command!='P' && Command!='I')
            dummy_printf("%s%s ",Cmd->DisablePercentage ? " ":"\b\b\b\b\b ",St(MOk));
#endif
        }
        else
        {
