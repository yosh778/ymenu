...
float JQuad::getWidth()  
{ 
	return mWidth;
}

float JQuad::getHeight()  
{ 
	return mHeight;
}
...
void JRenderer::EnableTextureFilter(bool flag)
{
	...
	// If BeginScene already set smoothing, reset it now, before next render
	if (mBeganScene)
	{
...
static void PNGCustomReadDataBin(png_structp png_ptr, png_bytep data, png_size_t length)
{
   png_size_t check;

   void *binData = (void*)png_ptr->io_ptr;

   memcpy(data, binData, length);
   binData = (void*)((u32)binData + length);

   png_ptr->io_ptr = binData;
}
...
JTexture* JRenderer::LoadTexture(const char* filename, int mode, int textureMode, void* binData, unsigned int binSize, bool dxppng)
{
    ...
    else if(strstr(filename, ".png")!=NULL || strstr(filename, ".PNG")!=NULL || binData!=NULL)
    {
        textureMode = TEXTURE_FORMAT; //textureMode not supported in PNG yet
        if (!dxppng)ret = LoadPNG(textureInfo, filename, mode, textureMode, binData);
        //else	ret = LoadDXPNG(textureInfo, filename, mode, binData, binSize);
        if (ret < 0)
        {
            char buf[512];
...
// Load PNG as texture
//------------------------------------------------------------------------------------------------
int JRenderer::LoadPNG(TextureInfo &textureInfo, const char* filename, int mode, int textureMode, void* binData)
{
  ...
  if (png_ptr == NULL) {
    if (binData == NULL)	fileSystem->CloseFile();
    return JGE_ERR_PNG;
  }
  ...
  if (binData == NULL)	png_set_read_fn(png_ptr, (png_voidp)fileSystem, PNGCustomReadDataFn);
  else	png_set_read_fn(png_ptr, (png_voidp)binData, PNGCustomReadDataBin);
...
/*
 * vMenu uses this lib to load pngs, but after some tests it didn't show any real speed improve, even with DXPPNG_MODE_GPU mode
 * TODO : Fix image layout + add swizzling
 *
 */

/*
int JRenderer::LoadDXPNG(TextureInfo &textureInfo, const char* filename, int mode, void* binData, unsigned int binSize)
{
	...
	{
		params.srcLength = binSize;
		params.src = binData;
	}

	params.mode = DXPPNG_MODE_RAW;//DXPPNG_MODE_GPU;
  ...
  int texWidth = png.widthN2;
  int texHeight = png.heightN2;
  ...
  {
    textureInfo.mBits = (u8 *)bits;
    textureInfo.mWidth = png.width;
    textureInfo.mHeight = png.height;
    textureInfo.mTexWidth = texWidth;
    textureInfo.mTexHeight = texHeight;
    textureInfo.mVRAM = videoRAMUsed;
    JLOG("-- OK -- JRenderer::LoadPNG");
